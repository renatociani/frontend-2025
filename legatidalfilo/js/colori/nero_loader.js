// Carica le magliette nere dalla pagina
document.addEventListener('DOMContentLoaded', async () => {
    const container = document.getElementById('nero-products');
    
    if (!container) return; // Se non siamo nella pagina nera, esci
    
    try {
        // Carica il file JSON con le frasi
        const response = await fetch('json/scritte_nero.json');
...
function createProductCard(frase, prezzo, index) {
    ...
    card.innerHTML = `
        <div class="product-image">
            <i class="fa-solid fa-shirt tshirt-icon" style="color: #1a1a1a; font-size: 11rem !important;"></i>
        </div>
        <div class="product-info">
            <h3 class="product-title">Nero #${index + 1}</h3>
