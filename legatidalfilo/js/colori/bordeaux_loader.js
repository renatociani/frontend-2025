// Carica le magliette bordeaux dalla pagina
document.addEventListener('DOMContentLoaded', async () => {
    const container = document.getElementById('bordeaux-products');
    
    if (!container) return;
    
    try {
        const response = await fetch('/json/scritte_bordeaux.json');
...
function createProductCard(frase, prezzo, index) {
    ...
    card.innerHTML = `
        <div class="product-image">
            <i class="fa-solid fa-shirt tshirt-icon" style="color: #8b0000; font-size: 11rem !important;"></i>
        </div>
        <div class="product-info">
            <h3 class="product-title">Bordeaux #${index + 1}</h3>
